...
<form action="{{ route('profile.update', ['user_id' => $user->id]) }}" method="POST" name="$jobs" id="edit_profile"
    ...
    <div class="container my-3 p-3 col-5 bg-light">
        ...
        <div class="row form-group m-4">
            ...
            <input class="form-control" id="title" type="text" name="nickname"
                value="{{old('nickname', $user->profile->nickname)}}" placeholder="ニックネームを書いて下さい。">
        </div>
        ...
        <fieldset class="m-4">
            ...
            <input class="col-3" type="number" name="year" id="year" placeholder="YYYY" 　size="8" maxlength="4"
                value="{{ old('year', substr(intval($user->profile->birth_of_date), 0, 4)) }}">
            年
            <input class="col-3" type="number" name="month" id="month" placeholder="MM" size="4" maxlength="2"
                value="{{ old('month', ltrim(substr(intval($user->profile->birth_of_date), 4, 2), '0')) }}">
            月
            <input class="col-3" type="number" name="day" id="day" placeholder="DD" size="4" maxlength="2"
                value="{{ old('day', ltrim(substr(intval($user->profile->birth_of_date), 6, 2), '0')) }}">
            日
        </fieldset>
        ...
        <div class="row form-group m-4">
            ...
            <input class="form-control" type="text" name="job" value="{{old('job', $user->profile->job)}}"
                placeholder="職業を書いて下さい。">
        </div>

        <label class="pt-2 mx-4" for="body">{{ __('コメント：') }}</label>
        <div class="form-group mx-4">
            <textarea class="body p-2" id="comment" name="comment"
                placeholder="コメントを書いて下さい。">{{old('comment', $user->profile->comment)}}</textarea>
        </div>
